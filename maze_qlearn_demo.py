...
Controls:
    ...
    T: Train Q-learning (progress printed in console)
...
# Q-learning hyperparams (you can change these before training)
ALPHA = 0.7             # learning rate
GAMMA = 0.95            # discount factor
EPSILON_START = 1.0     # initial exploration
EPSILON_END = 0.05
EPSILON_DECAY = 0.9995  # per episode
EPISODES = 4000         # number of training episodes (reasonable default)
...
def draw_grid(screen, env: MazeEnv, agent_pos=None, path=None, font=None):
    ...
    if font:
        lines = [
            ...
            "Keys: T = Train    R = Run    C = Clear walls    M = Random maze    Esc = Quit",
        ]
...
def main():
    ...
    pygame.display.set_caption("Maze Q-learning Demo")
            ...
            if event.type == pygame.KEYDOWN:
                ...
                if event.key == pygame.K_t:
                    ...
                    if env.start is None or env.goal is None:
                        ...
                        message = "Training... (see console)."
                        draw_grid(screen, env, font=font)
                        pygame.display.flip()
                        # run training (blocking) - prints progress to console
                        try:
                            q_table = train_q_learning(env, episodes=EPISODES, alpha=ALPHA, gamma=GAMMA,
                                                       eps_start=EPSILON_START, eps_end=EPSILON_END,
                                                       eps_decay=EPSILON_DECAY, max_steps=MAX_STEPS_EP,
                                                       render_callback=render_callback_progress, render_every=500)
                            training_done = True
                            message = "Training finished. Press R to run the policy."
